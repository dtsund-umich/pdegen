...
numx = int(math.ceil(len(pdes)**0.5))
numy = int(math.floor(len(pdes)**0.5))
...
for i in range(len(pdes)):
    if len(pdes[i].argnames) == 2:
        ax[i%numx][i/numx].axis('off')
        #pdes[i].plot_data(fig.add_subplot(2,2,i+1,projection='3d'),0)
        pdes[i].plot_data(fig.add_subplot(numx,numy,i+1,projection='3d'),0)
    else:
        pdes[i].plot_data(ax[i%numx][i/numx],0)
...
def update(val):
    ...
    for i in range(len(pdes)):
        if len(pdes[i].argnames) == 2:
            pdes[i].plot_data(fig.add_subplot(2,2,i+1,projection='3d'),t)
        else:
            pdes[i].plot_data(ax[i%numx][i/numx],t)
